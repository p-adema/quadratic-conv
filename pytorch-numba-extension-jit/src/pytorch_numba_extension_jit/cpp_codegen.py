    ...
    def prepare_args(
        ...
        asserts.extend(
            (
                ...
                f"TORCH_CHECK({self.name}.device().type() == at::DeviceType::CUDA);",
            )
        )
...
def _kernel_invocation(name: str, use_runtime_api: bool):
    ...
    CHK(cudaLaunchKernel((void*)kernel, 
...
"""
    ...
    CHK(cuLaunchKernel(kernel, blocksPerGrid, 1, 1, threadsPerBlock, 1, 1,
                       0, 0, args, NULL));
"""
    )
...
namespace ptex_jit {{
    ...
    {_return_type(outputs)} kernel_{name}({", ".join(parameters)}) {{
        ...
        unsigned int blocksPerGrid = (({n_threads}) + threadsPerBlock - 1)
                                        / threadsPerBlock;
